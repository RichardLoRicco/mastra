...
2. **Automatic Inheritance**: Child spans automatically inherit the TraceState from their parent, ensuring consistent metadata across the entire trace
3. **Metadata Precedence**: Explicit metadata passed to span options always takes precedence over extracted metadata

This approach ensures that all spans in a trace—including agent runs, LLM calls, tool executions, and workflow steps—receive the same contextual metadata without manual intervention.
