...
describe('DatadogExporter', () => {
  ...
  describe('token usage normalization', () => {
    ...
    it('forwards promptCacheHitTokens from v4 format as cachedInputTokens', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({
        type: SpanType.MODEL_GENERATION,
        attributes: {
    ...
    it('preserves provided totalTokens without recomputing', async () => {
      const exporter = new DatadogExporter({ mlApp: 'test', apiKey: 'test-key' });
      const span = createMockSpan({
        type: SpanType.MODEL_GENERATION,
        attributes: {
          usage: {
            inputTokens: 100,
            outputTokens: 50,
            totalTokens: 200, // Intentionally different from 100+50=150
          },
        },
      });
      ...
      expect(mockAnnotate).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          metrics: expect.objectContaining({
            totalTokens: 200, // Should preserve provided value, not recompute to 150
          }),
        }),
      );
    });
